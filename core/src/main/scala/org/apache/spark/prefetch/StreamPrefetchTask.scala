...
class StreamLoader(conf: SparkConf, partition: StreamPrefetchPartition) {
  ...
  def generateChunkedByteBuffer: ChunkedByteBuffer = {
    ...
    val ins = fs_.open(partition.path)
...
class StreamPrefetchTask(blockManager: BlockManager)
  ...
  def startPrefetchTask(meta: StreamMeta): (Long, Long) = {
    ...
    blockManager.putBytes(meta.blockId, chunkedByteBuffer, meta.level)
    (load_end - load_start, System.currentTimeMillis() - load_end)
  }
}
